...
\begin{itemize}
    ...
    \item \textbf{Expert Computation}: apply the SwiGLU activation~\cite{shazeer2020glu} and two matrix multiplications, often implemented with grouped general matrix multiplication (GroupGEMM~\cite{gale2023megablocks}) to improve hardware utilization.
...
\end{equation*}
The size of $\mathcal{U}_L$ determines how many experts must be materialized during execution, directly impacting the memory-access footprint, communication volume, and the efficiency of grouped execution.
...
Given the model's output logits $\mathbf{z}_i$ over the vocabulary $\mathcal{V}$ at step $i$, let $\mathbf{p}_i=\operatorname{softmax}(\mathbf{z}_i)$ denote the corresponding predictive distribution. Its Shannon entropy is defined as:
\begin{equation*}
...
\begin{cases}
k, & \psi_i \ge a_k, \\
j, & a_j \le \psi_i < a_{j+1},\quad j=1,\ldots,k-1, \\
0, & \psi_i < a_1,
\end{cases}
\end{equation}
...
At the sequence level, the key effect is dynamic: stronger approximation in one verification round tends to induce more conservative routing in later rounds.

Let $r$ index successive verification rounds, each comprising a draft block and its target verification. Let $P_r$ denote the perplexity-derived signal used by the policy function in Sec.~\ref{sec:method:policy}, and write the corresponding routing budget as
$
\hat{k}_r = f(P_r),
$
...
After round $r$ commits an output prefix, the draft model generates the next candidate block from that prefix. Empirically, stronger Top-$k$ reduction tends to increase approximation error and raise predictive perplexity in subsequent verification rounds.
At a coarse level, this gives the qualitative dependency
\begin{equation}
\hat{k}_r \downarrow \quad \Longrightarrow \quad P_{r+1} \uparrow.
\end{equation}
Combined with the monotonic policy,
\begin{equation}
P_{r+1} \uparrow \quad \Longrightarrow \quad \hat{k}_{r+1} = f(P_{r+1}) \uparrow,
\end{equation}
this yields a negative-feedback pattern across verification rounds:
if routing is temporarily too aggressive, the resulting rise in perplexity pushes later rounds toward larger Top-$k$, reducing subsequent approximation.
...
Each integration preserves its framework's native semantics. The primary vLLM system experiments use the distribution-entropy signal in Eq.~(\ref{eq:preview-signal}) and retain base-Top-$k$-normalized routing weights after masking, as defined above. The HuggingFace prototype uses proposed-token perplexity, $1/P(S_i)$, and renormalizes retained weights after masking. Both instantiate the same pre-execution preview-to-budget interface.

\subsection{Vectorized Policy Execution}

Continuous batching may combine requests using different policies. We fuse draft-distribution entropy, per-request threshold lookup, and token-wise Top-$k$ assignment into a single Triton~\cite{tillet2019triton} kernel over all candidate tokens in the batch, providing batched device-side policy evaluation with minimal overhead.
...
The primary challenge lies in the device-side All-to-All collectives used for expert-parallel dispatch and combine. In implementations such as PPLX~\cite{nandor2025pplxkernels}, the workload partitioning assumes that each input token generates a fixed number of $k$ communication tokens. Under dynamic routing budgets, this leaves warps assigned to masked positions idle. We instead partition work according to the actual number of active assignments, allowing the communication kernel to exploit the reduced routing budget.
...
Table~\ref{tab:assignment_load} separates bottleneck work from relative imbalance in an EP-32 GSM8K sweep. Busiest-device work is relative to SpecDecode; imbalance is the maximum-to-mean device load after normalizing assignment counts by target-model decode layer positions. Although this imbalance rises modestly from $1.47\times$ to $1.55\times$, busiest-device work falls to $0.68\times$ and $0.59\times$ for Lossless and Optimum, respectively, consistent with lower Combine latency.
...
Imbalance (max/mean) & $1.47\times$ & $1.55\times$ & $1.55\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
This effect is complementary to system-level optimizations: while reduced routing lowers the cost of each verification step, higher acceptance increases the number of output tokens committed per target-model step.
...
\begin{figure*}[t]
    ...
    \includegraphics[width=0.625\textwidth]{pareto/legend.pdf}
...
Across settings, the width and sharpness of the quality plateau vary with the model and generation regime.
...
Across settings, we observe a quality plateau whose width and transition sharpness vary as the Top-$k$ decreases.
...
A serving system can realize this by selecting an offline-calibrated policy for the request's generation regime.
...
In particular, Harder Tasks Need More Experts~\cite{huang2024harder} trains a confidence-based router with a dynamic sparsity objective.
These approaches rely on intrinsic signals such as intermediate activations and require retraining or architectural changes. In contrast, Spec-K adapts pretrained MoE models using a pre-execution draft signal.
