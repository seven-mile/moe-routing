...
Unlike prior adaptive mechanisms that rely on intrinsic model states (e.g., intermediate activations or router confidence), RASP is distinguished by its use of external preview signals available before target computation, enabling training-free adaptation without an additional predictor or model pass.
...
Given a token representation $\mathbf{x}_{L,i}$ at layer $L$, a gating function produces routing scores over expert $j$:
\begin{equation*}
\mathcal{G}_{L,i,j} = \operatorname{Gate}(\mathbf{x}_{L,i})_j.
\end{equation*}
Under token-choice routing~\cite{lepikhin2020gshard}, the Top-$k$ experts are selected, and their outputs are combined to form the layer output.
...
\begin{itemize}
    ...
    \item \textbf{Expert Computation}: including SiLU-Mul activation operation from SwiGLU~\cite{shazeer2020glu} and 2 GEMMs, often implemented with grouped general matrix multiplication (GroupGEMM~\cite{gale2023megablocks}) to improve hardware utilization.
...
For a batch of tokens, the system workload depends on the set of activated experts across all tokens. Let $\mathcal{S}_{L,i}$ denote the selected experts for token $i$, and define the union:
\begin{equation*}
...
During verification, the target model processes $1+\gamma$ tokens in parallel (including one carried-over token from the previous step). In MoE models, each of these tokens may activate a different subset of experts. As a result, the effective expert set becomes
\begin{equation*}
\mathcal{U}_L^{\text{SD}} = \bigcup_i \bigcup_{j=1}^{1+\gamma} \mathcal{S}_{L,(i,j)},
\end{equation*}
which can be significantly larger than that of a single token. This increases both communication overhead in dispatch/combine and reduces the efficiency of grouped expert execution.
...
Given the model's output probability distribution $\mathbf{z}_i$ over the vocabulary $\mathcal{V}$ at step $i$, the Shannon entropy is defined as:
\begin{equation*}
H(\mathbf{z}_i) = -\sum_{w \in \mathcal{V}} P(w | \mathbf{z}_i) \log P(w | \mathbf{z}_i).
\end{equation*}

The corresponding token-level perplexity is derived as the exponential of the entropy:
\begin{equation*}
\text{PP}(\mathbf{z}_i) = \exp(H(\mathbf{z}_i)).
\end{equation*}

While entropy quantifies the average information content, perplexity provides a more intuitive physical interpretation: it represents the \textit{effective size} of the model's choice set. Specifically, a perplexity value of $K$ indicates that the model's current uncertainty is equivalent to a uniform distribution over $K$ discrete tokens. In the context of speculative decoding, these metrics characterize the "sharpness" of the draft model's distribution prior to verification, where lower perplexity implies significant information redundancy, suggesting that the distribution is more robust to lossy approximations or high-ratio compression during distributed inference.
...
Adaptive allocation seeks to minimize $\mathbb{E}[\mathcal{C}(\mathbf{A})]$ under a quality constraint, where the expectation is over the deployment workload and the constraint specifies an application metric relative to the fixed-allocation baseline.
...
For each candidate token $S_i$, let $\mathbf{z}_i$ denote its predictive distribution from the draft model.
We compute the preview signal as its perplexity:
\begin{equation}
\psi_i = \exp(H(\mathbf{z}_i)), \quad i = 1,\dots,\gamma.
\label{eq:preview-signal}
\end{equation}
...
Given the allocation plan $\mathbf{A}$, we realize dynamic routing by modifying the standard Top-$k$ gating mechanism in each MoE layer.
Here, $k$ is the model's trained base routing budget and an upper bound: Spec-K removes lower-ranked assignments but never activates experts beyond the base Top-$k$.
For a token $t_i$ with hidden state $\mathbf{x}_{L,i}$ at layer $L$, let $e_{L,i,r}$ denote the expert at rank $r$ after sorting the selected experts by gate score. Standard routing computes the corresponding weights $w_{L,i,r} = \operatorname{Norm}(\operatorname{TopK}(\operatorname{Gate}(\mathbf{x}_{L,i}), k))_r$ for ranks $r=1,\ldots,k$.
Spec-K applies a straightforward zero-masking based on the allocated budget $\hat k_i$:
\begin{equation}
\hat{w}_{L,i,r} =
\begin{cases}
w_{L,i,r}, & r\le\hat{k}_i\\
0, & \text{otherwise}
\end{cases}
\end{equation}
This modification ensures that tokens with a reduced budget $\hat k_i$ activate only experts $e_{L,i,1},\ldots,e_{L,i,\hat k_i}$. Our primary vLLM path retains the base-Top-$k$-normalized weights and does not renormalize after masking.
...
Consequently, the resource cost of a verification step satisfies $\mathcal{C}(\mathbf{A}) \propto \sum_i \hat k_i$.
...
To optimize over this constrained space, we use a positive scale $s=a_k$ and ratios $b_i = a_i / a_{i+1} \in [0, 1]$. The thresholds are reconstructed recursively as $a_i=b_i a_{i+1}$, so the ordering is ensured by construction while $s$ preserves the absolute signal scale.

We use a standard multi-objective Bayesian optimization procedure to explore this space, implemented with Gaussian process-based optimization in BoTorch~\cite{balandat2020botorch} and a hypervolume-based acquisition function.
...
This optimization is an offline calibration step. At serving time, the selected thresholds are fixed, and runtime execution consists only of the piecewise policy and routing mask; BoTorch, per-batch search, and additional prediction models are not on the inference path.
...
quality degradation. An instance of this (DeepSeek-R1 w/ MTP) is shown in our evaluation 
...
We therefore focus on two tractable views of the same behavior: a per-step view based on \emph{local sensitivity} in auto-regressive decoding, and a sequence-level view based on \emph{self-regulation} under uncertainty-aware routing.

\subsubsection{Local Sensitivity of Auto-Regressive Decoding}
...
This choice can be understood through a basic robustness property of auto-regressive decoding.

Consider decoding step $t$ under greedy generation.
Let $\mathbf{z}_t \in \mathbb{R}^{V}$ denote the full-model logits over the vocabulary, and let
$
y_t^\star = \arg\max_i \mathbf{z}_t(i)
$
...
\begin{equation}
m_t = \mathbf{z}_t(y_t^\star) - \max_{j \neq y_t^\star} \mathbf{z}_t(j).
\end{equation}
...
$
\arg\max_i \hat{\mathbf{z}}_t(i) = y_t^\star.
$
...
Each integration preserves its framework's native semantics. The primary vLLM system experiments use the distribution-entropy signal in Eq.~(\ref{eq:preview-signal}) and retain base-Top-$k$-normalized routing weights after masking, as defined above. The HuggingFace prototype uses proposed-token surprisal, $1/P(S_i)$, and renormalizes retained weights after masking. Both instantiate the same pre-execution preview-to-budget interface.

% \subsection{Vectorized Policy Execution}

% In serving systems that employ continuous batching~\cite{yu2022orca}, allocation policies must be computed per request. A naïve implementation would launch a separate kernel for each request's piecewise policy evaluation, incurring significant kernel launch overhead and potential host--device synchronization stalls. To eliminate this overhead from the critical path, we fuse the entire policy computation into a single Triton~\cite{tillet2019triton} kernel that operates over all tokens in the batch concurrently. With this vectorized implementation, policy calculation accounts for less than $0.1\%$ of total execution time in typical deployment configurations.

\subsection{Adaptive Kernel Support}

Realizing the theoretical speedup of reduced routing budgets requires kernel implementations that are aware of inactive computation. For the expert computation itself, GroupGEMM kernels are natively designed to handle varying token counts per expert. Section~\ref{sec:evaluation:end2end} quantifies both the resulting absolute critical-path reduction and the modest change in relative skew.

The primary challenge lies in the device-side All-to-All collectives used for expert-parallel dispatch and combine. In implementations such as PPLX~\cite{nandor2025pplxkernels}, the workload partitioning assumes that each input token generates a fixed number of $k$ communication tokens. When routing budgets vary across tokens under Spec-K, this fixed partitioning leads to warp-level imbalance. We augment the workload partitioning with adaptive rebalancing that accounts for per-token budget variations. Microbenchmarks confirm that this adjustment brings observed communication latency into closer alignment with the theoretical reduction in communication volume.
...
We adopt a distributed inference setup with data parallelism (DP) for dense layers and expert parallelism (EP) for MoE layers, without tensor parallelism. We denote configurations as EP-$k$, where attention layers use DP-$k$ and MoE experts are partitioned across $k$ devices.

Our implementation is based on vLLM~v0.16.0 with Triton-based GroupGEMM kernels and DeepEP in low-latency mode (IBGDA). Prefix caching is disabled to ensure consistent performance and accuracy measurement. Speculative decoding uses padded drafter batches to minimize scheduling overhead. All methods, including the baseline, share the same system stack for fair comparison.

\subsubsection{Models and Drafting Methods}

We evaluate across multiple model families, including Qwen3, DeepSeek, and GLM, covering scales from 30B to 671B parameters and both shared and non-shared expert architectures.
...
The Lossless and Optimum policies achieve up to $1.44\times$ and $1.72\times$ speedup over the SpecDecode baseline, respectively. 
...
\begin{figure}[!t]
    ...
    Spec-K reduces total latency in both settings: GroupGEMM and Combine decrease in both cases, while Dispatch decreases in the inter-node EP-32 case and remains effectively unchanged in the intra-node EP-8 case.}
...
To examine whether Spec-K's benefits depend on the GPU software stack used above, we additionally evaluate its vLLM-Ascend implementation~\cite{huawei2024vllmascend} on an Ascend NPU platform. We use Qwen3-30B-A3B as the target model, Qwen3-0.6B as the standalone draft model, ShareGPT requests, and seven draft candidates per verification step. At each offered load, Spec-K and SpecDecode use the same model, workload, software stack, and deployment configuration. We report throughput only relative to the corresponding baseline and omit detailed hardware configuration; these measurements provide portability evidence rather than an absolute comparison with Figure~\ref{fig:throughput_end2end}.
...
\begin{axis}[
    ...
    symbolic x coords={128,256,384,512},
    xtick=data,
    ytick={0,0.5,1.0,1.5},
    axis lines*=left,
    enlarge x limits=0.16,
    tick align=outside,
    nodes near coords={\pgfmathprintnumber[fixed,precision=2,fixed zerofill]{\pgfplotspointmeta}$\times$},
    point meta=y,
    every node near coord/.append style={font=\footnotesize},
    legend style={font=\footnotesize, draw=none, at={(0.5,1.02)}, anchor=south, legend columns=2},
]
\addplot[fill=blue!55, draw=blue!70!black] coordinates {(128,1.14) (256,1.29) (384,1.32) (512,1.10)};
\addlegendentry{Spec-K}
\addplot[black, dashed, mark=none, sharp plot, nodes near coords={}] coordinates {(128,1.00) (256,1.00) (384,1.00) (512,1.00)};
\addlegendentry{SpecDecode}
\end{axis}
\end{tikzpicture}
\caption{Normalized end-to-end throughput on an Ascend NPU platform. Each result is normalized to SpecDecode at the same concurrency and deployment configuration; absolute throughput and detailed hardware configuration are omitted. Spec-K improves throughput at every tested load. The 512-request setting experiences KV-cache capacity pressure.}
\label{fig:ascend_throughput}
\end{figure}

As shown in Figure~\ref{fig:ascend_throughput}, Spec-K improves end-to-end throughput by $1.14\times$, $1.29\times$, and $1.32\times$ at 128, 256, and 384 concurrent requests, respectively. At 512 concurrent requests, where KV-cache capacity pressure constrains the deployment, Spec-K still achieves a $1.10\times$ improvement. These results show that the same preview-guided dynamic Top-$k$ mechanism transfers to a distinct accelerator and serving ecosystem without retraining or modifying model weights.
...
Communication behavior depends on the deployment regime. In the inter-node EP-32 case, Dispatch latency falls with routing volume; in the intra-node EP-8 case it is effectively unchanged. Combine latency decreases in both settings but remains substantial because it includes inter-device synchronization and therefore reflects load imbalance in MoE computation.
In an EP-32 GSM8K runtime sweep, the maximum per-device token--expert assignment volume---the synchronization-relevant critical-path load---falls to $0.68\times$ and $0.59\times$ of the baseline for the Lossless and Optimum policies, respectively. Although normalized skew increases modestly as aggregate work contracts, the absolute critical-path work decreases, consistent with the observed Combine-latency reduction.
...
Remarkably, lower average Top-$k$ consistently yields higher acceptance along the evaluated Spec-K frontier. This reveals a previously overlooked systems effect: adaptive target-side routing improves both sides of speculative verification, reducing the cost of each verification step while increasing the number of draft tokens accepted per step. The Pareto evaluation below identifies the operating region where this gain is retained without material target-quality loss.

This effect is complementary to system-level optimizations: while reduced routing lowers the cost of each verification step, higher acceptance increases the number of tokens processed per step. 
Together, these factors contribute to the overall throughput improvement observed in Section~\ref{sec:evaluation:end2end}.

\subsection{Quality--Efficiency Tradeoff and Generalization}

\subsubsection{Comparison with Allocation Baselines}

To isolate the value of the preview signal, we construct a fixed lower-layer policy that reduces the first $n$ MoE layers to a constant $k$ and preserves the trained Top-$k$ in all later layers, without using draft perplexity. On Qwen3-30B-A3B with Qwen3-0.6B and GSM8K-COT, every tested lower-layer policy is dominated by the Spec-K frontier. At the high-quality end, a fixed policy scores 88.93 at average Top-$k$ 6.01, whereas Spec-K scores 91.36 at Top-$k$ 5.97. The strongest fixed-policy run scores 90.14 at Top-$k$ 5.73, while Spec-K scores 90.45 at the lower Top-$k$ of 5.45. Thus, unconditional layer reduction exposes some redundancy, but preview-guided allocation finds a better quality--efficiency tradeoff in the same model and evaluation stack.

Lynx~\cite{gupta2024lynx}, a complementary batch-aware expert-remapping system, reports $1.14\times$ and $1.09\times$ throughput improvements at EP degrees 2 and 4, respectively, in its evaluation setup while keeping accuracy within one percentage point. Lynx uses target-side router signals to remap experts at batch level, whereas Spec-K uses pre-execution draft signals to allocate token-wise routing budgets; the two methods optimize complementary dimensions of MoE serving.
...
Main configuration used throughout the paper
    ...
    \hfil
    \subfloat[{\parbox[c]{0.9\linewidth}{\centering
Qwen3-30B-Instruct-2507, EAGLE, GSM8K \\
Non-reasoning model on a reasoning calibration
    ...
    \hfil
    \subfloat[{\parbox[c]{0.9\linewidth}{\centering
Qwen3-30B-Instruct-2507, EAGLE, IFEval \\
...
Limited routing redundancy for $k=4$
    ...
    \caption{Quality--efficiency Pareto frontiers across diverse model architectures, drafting methods, and tasks. The gray region and black error bar indicate the variability (±1 std) of the baseline.
...
HumanEval {\scriptsize (P@64)} & 54.88 / 8 & 60.37 / 5.51 & 67.68 / 4.86 \\
...
As shown in Table~\ref{tab:transfer_accuracy}, the Lossless policy remains within the observed variation or a negligible drop across tasks, indicating that conservative allocation strategies generalize well.
...
These differences imply that phase-aware allocation policies may further improve performance in practice.
...
These approaches rely on intrinsic signals such as intermediate activations and require retraining or architectural changes.

Training-free methods adapt computation at inference time. 
Token pruning and KV-cache reduction (e.g., SnapKV~\cite{li2024snapkv}) reduce attention cost, while MoE-specific methods such as Lynx~\cite{gupta2024lynx} prune experts based on routing confidence. 
These approaches typically rely on signals observed during execution and operate at token-, block-, or batch-level.

In contrast, our method performs \emph{pre-execution} adaptation using preview signals, enabling token-level resource allocation before the target forward pass, suggesting a new paradigm for guiding adaptive inference.

\section{Discussion and Conclusion}

Spec-K is an instantiation of a broader principle: \emph{resource allocation should be decided before execution, guided by predictive signals}. Formalized as RASP, this decouples \emph{difficulty estimation} from \emph{computation allocation}, enabling anticipatory adaptation using preview signals available prior to the target forward pass, without modifying model internals or adding a separate predictor or model pass.

Our results reveal a consistent phenomenon: reducing the routing budget can improve both efficiency and speculative acceptance. This exposes a previously overlooked coupling between target-side expert allocation and speculative verification: activating more experts does not necessarily improve draft--target alignment, while preview-guided sparsification can reduce verification cost and increase acceptance. The observed Pareto structure---a wide plateau followed by a sharp degradation---indicates substantial redundancy for typical tokens, while a small subset of difficult tokens defines the capacity boundary. Spec-K exploits this asymmetry by reallocating resources toward high-uncertainty regions.

This approach has several limitations. Its effectiveness depends on preview signal quality; mismatch between draft uncertainty and true difficulty can lead to suboptimal allocation. Different generation regimes also exhibit varying sensitivity: policies calibrated on reasoning tasks may be over-aggressive for instruction-following workloads. Finally, Spec-K operates at token-level granularity with a uniform policy across layers, leaving finer-grained control unexplored.
