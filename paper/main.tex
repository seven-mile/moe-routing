...
For a token representation $\mathbf{x}_{L,i}$ at layer $L$, let
\begin{equation*}
\mathcal{R}_{L,i} = \operatorname{TopK}(\operatorname{Gate}(\mathbf{x}_{L,i}), k)
\end{equation*}
denote the standard ordered routing output under token-choice routing~\cite{lepikhin2020gshard}. Its entries pair the selected expert identifiers with their routing weights in descending gate-score order; the corresponding expert outputs are combined to form the layer output.
...
For a token batch indexed by $\mathcal{T}$, the system workload also depends on the set of activated experts across all tokens. Writing $\operatorname{Experts}(\mathcal{R})$ for the expert identifiers in an ordered route, define the union:
\begin{equation*}
\mathcal{U}_L = \bigcup_{i\in\mathcal{T}} \operatorname{Experts}(\mathcal{R}_{L,i}).
\end{equation*}
...
During verification, the target model processes $1+\gamma$ tokens in parallel (including one carried-over token from the previous step). In MoE models, each of these tokens may activate a different subset of experts. For a request $b$ in batch $\mathcal{B}$, let $i=1,\dots,1+\gamma$ index positions in its verification block and let $\mathcal{R}_{L,b,i}$ denote the corresponding ordered route. The effective expert set becomes
\begin{equation*}
\mathcal{U}_L^{\text{SD}} = \bigcup_{b\in\mathcal{B}} \bigcup_{i=1}^{1+\gamma} \operatorname{Experts}(\mathcal{R}_{L,b,i}),
\end{equation*}
...
We instantiate RASP for MoE routing with \textbf{Spec-K}, a dynamic Top-$k$ allocation mechanism driven by speculative preview. We present Spec-K for one request and one verification block; batched serving applies the same policy independently to each request. Accordingly, $i=1,\dots,1+\gamma$ indexes positions within that block.
...
Given the allocation plan $\mathbf{A}$, we realize dynamic routing by modifying the standard Top-$k$ gating mechanism in each MoE layer. To describe this operation, we fix an arbitrary MoE layer and suppress the request and layer indices, writing $\mathcal{R}_i$ for the ordered route $\mathcal{R}_{L,b,i}$ defined in Sec.~\ref{sec:bg:moe-serving}. Spec-K retains the prefix selected by the allocated budget:
\begin{equation}
\hat{\mathcal{R}}_i = \operatorname{Prefix}_{\hat k_i}(\mathcal{R}_i),
\quad i=1,\dots,1+\gamma,
\end{equation}
where $\operatorname{Prefix}_m$ retains the first $m$ expert-weight entries of an ordered route. Thus, a token with budget $\hat k_i$ activates only its top $\hat k_i$ experts while preserving their routing order and associated weights.
