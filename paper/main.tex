...
\rhead{}
\chead{}
\lfoot{\footnotesize{SC26, November 15-20, 2026, Chicago, Illinois, USA
...
Continuous batching may combine requests that use different fixed policies. Evaluating these policies request by request would introduce repeated kernel launches and host--device coordination. We instead fuse draft-distribution entropy, per-request threshold lookup, and token-wise Top-$k$ assignment into a single Triton~\cite{tillet2019triton} kernel over all candidate tokens in the batch. The kernel produces routing budgets directly on device; the online path contains neither policy search nor an additional prediction pass.
...
To examine whether Spec-K's benefits depend on the GPU software stack used above, we additionally evaluate its vLLM-Ascend implementation~\cite{huawei2024vllmascend} on an Ascend NPU platform. We use Qwen3-30B-A3B as the target model, Qwen3-0.6B as the standalone draft model, ShareGPT requests, and seven draft candidates per verification step. At each offered load, Spec-K and SpecDecode use the same model, workload, software stack, and deployment configuration. We report only normalized throughput and omit detailed hardware configuration; these measurements provide portability evidence rather than an absolute comparison with Figure~\ref{fig:throughput_end2end}.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{ascend/normalized_throughput.pdf}
\caption{End-to-end throughput on an Ascend NPU platform, normalized to SpecDecode at 128 concurrent requests. Labels above the Spec-K Optimum bars report speedup over SpecDecode at the same concurrency. Absolute throughput and detailed hardware configuration are omitted. The 512-request setting experiences KV-cache capacity pressure.}
\label{fig:ascend_throughput}
\end{figure}

As shown in Figure~\ref{fig:ascend_throughput}, the Spec-K Optimum policy improves end-to-end throughput by $1.14\times$, $1.29\times$, and $1.32\times$ at 128, 256, and 384 concurrent requests, respectively. At 512 concurrent requests, where KV-cache capacity pressure constrains the deployment, it still achieves a $1.10\times$ improvement. These results show that the same preview-guided dynamic Top-$k$ mechanism transfers to a distinct accelerator and serving ecosystem without retraining or modifying model weights.
...
Table~\ref{tab:assignment_load} separates relative imbalance from the absolute work on the critical path in an EP-32 GSM8K runtime sweep. Although max/mean skew increases modestly as aggregate work contracts, the maximum per-device assignment volume falls to $0.68\times$ and $0.59\times$ of the baseline for Lossless and Optimum, respectively. This absolute critical-path reduction is consistent with the observed Combine-latency improvement.

\begin{table}[t]
\caption{EP-32 assignment load relative to SpecDecode.}
\label{tab:assignment_load}
\centering
\small
\begin{tabular}{lccc}
\toprule
...
\midrule
Critical-path assignments & $1.00\times$ & $0.68\times$ & $0.59\times$ \\
Max/mean assignment skew & $1.00\times$ & $1.06\times$ & $1.05\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
To isolate the value of the preview signal, we construct a fixed lower-layer policy that reduces the first $n$ MoE layers to a constant $k$ and preserves the trained Top-$k$ in all later layers, without using draft perplexity. On Qwen3-30B-A3B with Qwen3-0.6B and GSM8K-COT, every tested lower-layer policy is dominated by the Spec-K frontier. The representative cross in Figure~\ref{fig:frontiers}(a) shows the high-quality comparison: the fixed policy scores 88.93 at average Top-$k$ 6.01, whereas Spec-K scores 91.36 at the lower Top-$k$ of 5.97. Unconditional layer reduction exposes some redundancy, but preview-guided allocation finds a better quality--efficiency tradeoff in the same model and evaluation stack.

For context across systems, Lynx~\cite{gupta2024lynx} reports $1.14\times$ and $1.09\times$ throughput improvements on Qwen2-57B-A14B-Instruct with GSM8K at TP-2/EP-2 and TP-4/EP-4, respectively, while keeping exact-match accuracy within one percentage point. Lynx remaps target-side routing decisions to reduce the active-expert set of a batch; Spec-K instead uses pre-execution draft signals to allocate token-wise routing budgets. The two methods therefore optimize complementary dimensions of MoE serving.
...
Limited routing redundancy for $k=4$
    ...
    \caption{Quality--efficiency Pareto frontiers across diverse model architectures, drafting methods, and tasks. The gray region and black error bar indicate the variability (±1 std) of the baseline; the gold cross in (a) marks a representative fixed lower-layer policy.
