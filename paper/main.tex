...
Unlike prior adaptive mechanisms that rely on intrinsic model states (e.g., intermediate activations or router confidence), RASP uses external preview signals available before target computation. In speculative decoding, these signals are derived from predictive distributions already produced by the draft model.
...
\end{cases}
\end{equation}
This modification ensures that tokens with a reduced budget $\hat k_i$ activate only experts $e_{L,i,1},\ldots,e_{L,i,\hat k_i}$. Our primary vLLM path retains the base-Top-$k$-normalized weights after masking.
...
The resulting Pareto frontier is computed during offline calibration. Serving uses a selected set of fixed thresholds and evaluates the corresponding piecewise policy for each token.
...
Continuous batching may combine requests using different policies. We fuse draft-distribution entropy, per-request threshold lookup, and token-wise Top-$k$ assignment into a single Triton~\cite{tillet2019triton} kernel over all candidate tokens in the batch. Across the serving configurations evaluated below, policy execution accounts for less than $0.1\%$ of total runtime.
...
We further evaluate Spec-K with vLLM-Ascend~\cite{huawei2024vllmascend} to study portability across accelerator and serving stacks. The experiment uses Qwen3-30B-A3B, a standalone Qwen3-0.6B draft model, ShareGPT requests, and seven draft candidates per verification step. Spec-K and SpecDecode share the same configuration at each offered load. Throughput is normalized to SpecDecode at 128 concurrent requests; the deployment configuration remains confidential.

\begin{figure}[!b]
\centering
\includegraphics[width=\linewidth]{ascend/normalized_throughput.pdf}
\caption{Normalized end-to-end throughput on an Ascend NPU platform. Values are normalized to SpecDecode at 128 concurrent requests; annotations report Spec-K Optimum speedup over SpecDecode at the corresponding load. The 512-request setting experiences KV-cache capacity pressure.}
...
Spec-K is an instantiation of a broader principle: \emph{resource allocation should be decided before execution, guided by predictive signals}. Formalized as RASP, this decouples \emph{difficulty estimation} from \emph{computation allocation} and reuses predictive distributions already produced during speculative decoding to guide the target forward pass.
