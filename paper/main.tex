...
% Queue the full-width quality result for the next page.
\begin{figure*}[t]
    \centering
...
This effect is complementary to system-level optimizations: reduced routing lowers verification cost, while higher acceptance commits more output tokens per target-model step.
Together, they drive the throughput gains in Section~\ref{sec:evaluation:end2end}.
...
\caption{Normalized end-to-end throughput on an Ascend NPU platform, using SpecDecode at 128 concurrent requests as the common reference. Labels report Spec-K Optimum speedup; the 512-request setting experiences KV-cache capacity pressure.}
