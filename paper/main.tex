...
For a token batch $\mathcal{T}$, the system workload depends on the set of activated experts across all tokens. Let $\mathcal{S}_{L}(x)$ denote the selected experts for token $x\in\mathcal{T}$, and define the union:
\begin{equation*}
\mathcal{U}_L = \bigcup_{x\in\mathcal{T}} \mathcal{S}_{L}(x).
\end{equation*}
...
During verification, the target model processes $1+\gamma$ tokens in parallel (including one carried-over token from the previous step). In MoE models, each of these tokens may activate a different subset of experts. For a request $b$ in batch $\mathcal{B}$, let $q=0,\dots,\gamma$ index positions in its verification block and let $\mathcal{S}_{L,b,q}$ denote the corresponding selected experts. The effective expert set becomes
\begin{equation*}
\mathcal{U}_L^{\text{SD}} = \bigcup_{b\in\mathcal{B}} \bigcup_{q=0}^{\gamma} \mathcal{S}_{L,b,q},
\end{equation*}
