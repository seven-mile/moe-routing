...
Continuous batching may combine requests using different policies. We fuse draft-distribution entropy, per-request threshold lookup, and token-wise routing-budget assignment into a single Triton~\cite{tillet2019triton} kernel over all candidate tokens, amortizing policy-evaluation launch overhead across the batch.
