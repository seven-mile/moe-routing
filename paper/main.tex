...
% CAMERA-READY TODO: Replace this block with the submitted author list in the
% original order; SC26 does not permit author-list changes after reviews.
...
This abstraction decouples preview generation from policy design. Extensions to resources such as attention sparsity or layer skipping remain future work.}
...
Beyond MoE routing, the framework may inform other forms of adaptive computation, such as sparse attention or layer skipping; this paper evaluates only the MoE instantiation.
...
The primary vLLM system experiments use the distribution-entropy signal in Eq.~(\ref{eq:preview-signal}) and retain base-Top-$k$-normalized routing weights after masking, as defined above. The HuggingFace prototype follows its assisted-generation interface by using proposed-token surprisal, $1/P(S_i)$, as the uncertainty signal and renormalizing retained weights after masking. These choices share the same preview-guided allocation interface, but we do not treat their numerical policies as interchangeable.
...
In an EP-32 GSM8K runtime sweep, the maximum per-device token--expert assignment volume---the synchronization-relevant critical-path load---falls to $0.68\times$ and $0.59\times$ of the baseline for the Lossless and Optimum policies, respectively. Relative skew can increase modestly because total work falls faster than the lightest device loads, but the absolute critical-path work decreases, consistent with the observed Combine-latency reduction.
...
Lower routing budgets are associated with higher acceptance in this sweep.}
...
As shown in Figure~\ref{fig:topk_vs_acceptance}, lower average Top-$k$ is consistently associated with higher acceptance in this sweep, allowing more draft tokens to be verified per step.
This correlation is an empirical result rather than evidence that sparsification itself causes better alignment; the quality measurements below remain necessary to rule out policies that merely imitate the draft model at the target model's expense.
...
To isolate the value of the preview signal, we compare Spec-K with a fixed lower-layer policy suggested by the reviewers. This baseline reduces the first $n$ MoE layers to a constant $k$ and preserves the trained Top-$k$ in all later layers, without using draft perplexity. On Qwen3-30B-A3B with Qwen3-0.6B and GSM8K-COT, every tested lower-layer policy is dominated by the Spec-K frontier. At the high-quality end, a fixed policy scores 88.93 at average Top-$k$ 6.01, whereas Spec-K scores 91.36 at Top-$k$ 5.97. The strongest fixed-policy run scores 90.14 at Top-$k$ 5.73, while Spec-K scores 90.45 at the lower Top-$k$ of 5.45. Thus, unconditional layer reduction exposes some redundancy, but preview-guided allocation finds a better quality--efficiency tradeoff in the same model and evaluation stack. These runs compare quality and routing budget only; they do not support a separate throughput claim.

Lynx~\cite{gupta2024lynx}, a complementary batch-aware expert-remapping system, reports $1.14\times$ and $1.09\times$ throughput improvements at EP degrees 2 and 4, respectively, while keeping accuracy within one percentage point. These cross-paper numbers are not directly comparable because Lynx uses different models, deployment conditions, and target-side router signals. The controlled lower-layer comparison above therefore provides the more direct test of Spec-K's preview signal.
...
Our results show that reducing the routing budget can improve efficiency and, at the evaluated operating points, coincide with higher speculative acceptance. The observed Pareto structure---a wide plateau followed by a sharp degradation---indicates substantial routing redundancy for typical tokens, while a smaller subset of difficult tokens defines the quality boundary. Spec-K exploits this asymmetry by allocating larger budgets to high-uncertainty regions; we do not claim that sparsification causally improves draft--target alignment.

This approach has two practical prerequisites: a sufficiently aligned draft--target pair and reducible routing redundancy in the target model and serving stack. Mismatch between draft uncertainty and true difficulty can lead to suboptimal allocation, while models with a small base Top-$k$ may offer little removable work. Different generation regimes also exhibit varying sensitivity: policies calibrated on reasoning tasks may be over-aggressive for instruction-following workloads. Finally, Spec-K operates at token-level granularity with a uniform policy across layers, leaving finer-grained control unexplored.
...
% CAMERA-READY TODO: Add verified sponsor/funding acknowledgments here.
OpenAI Codex~\cite{openai2026codex} assisted with language editing and organization throughout the final manuscript. The authors independently verified all technical claims, data, and citations and take full responsibility for the content.
