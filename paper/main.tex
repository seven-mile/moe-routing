...
Unlike prior adaptive mechanisms that rely on intrinsic model states (e.g., intermediate activations or router confidence), RASP is distinguished by its use of external preview signals available before target computation, enabling training-free and zero-overhead adaptation.
...
The predictive distribution of a language model provides a intrinsic measure of uncertainty regarding the next token. 
...
Adaptive allocation seeks to minimize $\mathbb{E}[\mathcal{C}(\mathbf{A})]$ under a quality constraint.
...
For a token $t_i$ with hidden state $\mathbf{x}_{L,i}$ at layer $L$, the standard routing computes gating weights $\mathcal{G}_{L,i,j} = \operatorname{Norm}(\operatorname{TopK}(\operatorname{Gate}(\mathbf{x}_{L,i}), k))_j$.
Spec-K applies a straightforward zero-masking based on the allocated budget $\hat k_i$:
\begin{equation}
\hat{\mathcal{G}}_{L,i,j} =
\begin{cases}
\mathcal{G}_{L,i,j}, & j\le\hat{k}_i\\
0, & \text{otherwise}
\end{cases}
\end{equation}
This modification ensures that tokens with a reduced budget $\hat k_i$ activate only the top $\hat k_i$ experts, leaving the rest unused without affecting the weight normalization of the active experts, if any.
...
To optimize over this constrained space, we reparameterize $\{a_i\}$ using
$b_i = a_i / a_{i+1} \in [0, 1]$, yielding a set of bounded and independent variables; the ordering of $\{a_i\}$ is then ensured by construction.
...
Realizing the theoretical speedup of reduced routing budgets requires kernel implementations that are aware of inactive computation. For the expert computation itself, GroupGEMM kernels are natively designed to handle varying token counts per expert, and we observe no significant degradation in load balance under Spec-K's adaptive routing budgets.
...
Spec-K reduces latency across all major components, including computation (GroupGEMM) and communication (Dispatch and Combine).
...
Figure~\ref{fig:overall_latency_breakdown} shows that Spec-K consistently reduces latency across all major components. GroupGEMM speedup exceeds the theoretical expectation from average Top-$k$, indicating improved kernel efficiency under smaller expert sets.

Communication overhead is also reduced. Dispatch latency closely follows the reduction in routing volume, while Combine latency improves more moderately. Notably, Combine latency substantially exceeds Dispatch latency in both settings, primarily due to inter-GPU synchronization, reflecting the overhead induced by load imbalance in MoE computation. The observed reduction in Combine latency indicates that Spec-K does not degrade EP load balance.
...
Non-reasoning model a reasoning calibration
}}]{
...
\caption{Cross-Task Accuracy Results of GSM8K-COT (Score / Top-$k$)}
...
Spec-K is an instantiation of a broader principle: \emph{resource allocation should be decided before execution, guided by predictive signals}. Formalized as RASP, this decouples \emph{difficulty estimation} from \emph{computation allocation}, enabling anticipatory adaptation using preview signals available prior to the target forward pass, without modifying model internals or incurring overhead.
