...
Unlike prior adaptive mechanisms that rely on intrinsic model states (e.g., intermediate activations or router confidence), RASP is distinguished by its use of external preview signals available before target computation, enabling lightweight, training-free adaptation.
...
Given a token representation $\mathbf{x}_{L,i}$ at layer $L$, a gating function produces a routing score for each expert $j\in\{1,\dots,n\}$:
\begin{equation*}
...
\begin{equation*}
\mathcal{U}_L^{\text{SD}} = \bigcup_b \bigcup_{q=1}^{1+\gamma} \mathcal{S}_{L,(b,q)},
\end{equation*}
where $b$ indexes requests in the batch and $q$ indexes positions within a verification block. This union can be significantly larger than that of a single token. This increases both communication overhead in dispatch/combine and reduces the efficiency of grouped expert execution.
...
\begin{equation*}
H(\mathbf{p}_i) = -\sum_{v \in \mathcal{V}} p_i(v) \log p_i(v).
\end{equation*}
...
Here, $k$ is the model's trained base routing budget and an upper bound: Spec-K may remove assignments from this set but never activates experts outside the base Top-$k$.
For a token $t_i$ at layer $L$, let $e_{L,i,m}$ denote the identifier of the expert with the $m$-th largest score $\mathcal{G}_{L,i,j}$, and let $w_{L,i,m}$ denote its normalized routing weight, for $m=1,\dots,k$. Thus, $m$ indexes entries in the ordered Top-$k$ output rather than experts in the model.
Spec-K applies a straightforward zero-masking based on the allocated budget $\hat k_i$:
\begin{equation}
\hat{w}_{L,i,m} =
\begin{cases}
w_{L,i,m}, & m\le\hat{k}_i\\
0, & \text{otherwise}
\end{cases}
\end{equation}
This modification retains only experts $e_{L,i,1},\dots,e_{L,i,\hat k_i}$. Our primary vLLM path keeps the weights normalized over the base Top-$k$ and does not renormalize after masking.
...
\begin{cases}
k, & \psi_i \ge a_k, \\
c, & a_c \le \psi_i < a_{c+1},\quad c=1,\ldots,k-1, \\
0, & \psi_i < a_1,
\end{cases}
\end{equation}
...
To optimize over this constrained space, we use a positive scale $s=a_k$ and ratios $b_i=a_i/a_{i+1}\in[0,1]$ for $i=1,\ldots,k-1$. The thresholds are reconstructed recursively as $a_i=b_i a_{i+1}$, so their ordering is ensured by construction while $s$ preserves the absolute signal scale.
...
Our primary vLLM implementation uses draft-distribution entropy from Eq.~(\ref{eq:preview-signal}) and retains base-Top-$k$-normalized routing weights after masking. The HuggingFace integration uses proposed-token perplexity, $1/p_i(S_i)$, and renormalizes the retained weights after masking. Both supply a token-wise preview signal to the same allocation-policy interface.

\subsection{Vectorized Policy Execution}

Continuous batching may combine requests using different policies. We fuse draft-distribution entropy, per-request threshold lookup, and token-wise Top-$k$ budget assignment into a single Triton~\cite{tillet2019triton} kernel over all candidate tokens in the batch, providing batched device-side policy evaluation with minimal overhead.

\subsection{Adaptive Kernel Support}

Realizing the theoretical speedup of reduced routing budgets requires kernel implementations that are aware of inactive computation. For the expert computation itself, GroupGEMM kernels are natively designed to handle varying token counts per expert, allowing the reduced assignment volume to translate into lower computation.
...
Reducing the routing budget consistently increases acceptance, indicating improved alignment between draft and target models under adaptive allocation.}
...
This suggests that more aggressive routing implicitly improves alignment between the draft and target models, allowing more draft tokens to be verified per step.
...
We further evaluate the Spec-K Optimum policy with vLLM-Ascend~\cite{huawei2024vllmascend} to study portability across accelerator and serving stacks. The experiment uses Qwen3-30B-A3B, a standalone Qwen3-0.6B draft model, ShareGPT requests, and seven draft candidates per verification step. Spec-K and SpecDecode share the same configuration at each offered load. To preserve deployment confidentiality, we report normalized throughput without hardware details or absolute performance.
...
In particular, Harder Tasks Need More Experts~\cite{huang2024harder} trains a confidence-based router with a dynamic sparsity objective. These approaches rely on intrinsic signals such as intermediate activations and require retraining or architectural changes. In contrast, Spec-K adapts pretrained MoE models using a pre-execution draft signal.
...
Spec-K is an instantiation of a broader principle: \emph{resource allocation should be decided before execution, guided by predictive signals}. Formalized as RASP, this decouples \emph{difficulty estimation} from \emph{computation allocation}, enabling anticipatory adaptation using preview signals available prior to the target forward pass, without modifying model internals and with minimal policy overhead.
